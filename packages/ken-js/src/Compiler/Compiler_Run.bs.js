  ...
  if (match !== undefined) {
    ...
    if (exit === 1) {
      if (IdConverter$BsKen.isFullId(r)) {
        ...
        return IdConverter$BsKen.toFullId($$package[/* baseId */1], $$package[/* resourceId */2], r);
      }
    }
...
function generateFactId(thingId, subjectId, $$package) {
  ...
  var isInSameBase = Graph_T$BsKen.Directory[/* root */5](subject) === $$package[/* baseId */1];
  var tagId = Option$Rationale.toExn("Expected fact to have fact ID at this point.", thingId[/* tag */1]);
  if (isInSameBase) {
    return subject + ("/_f/" + tagId);
  } else {
    return Option$Rationale.toExn("Expect Full Base Id Id", IdConverter$BsKen.toFullId($$package[/* baseId */1], $$package[/* resourceId */2], "_f/" + tagId));
  }
}
...
function showFacts(g) {
  return $$Array.map(Compiler_AST$BsKen.factToJs, $$Array.of_list(g[/* facts */0]));
}
